      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border p-6">
                  ...
                  {row.slice(0, 2).map((seat) => (
                    <button key={seat.seat_number} disabled={!seat.is_available} onClick={() => setSelectedSeat(seat.seat_number)}
                      className={`w-10 h-10 rounded text-xs font-medium transition ${
                        selectedSeat === seat.seat_number ? "bg-green-600 text-white ring-2 ring-green-300"
                        : seat.is_available ? "bg-blue-100 hover:bg-blue-200 text-blue-800" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
          ...
          <div className="flex justify-center gap-4 mt-6 text-xs text-gray-500">
            ...
            <span className="flex items-center gap-1"><span className="w-4 h-4 bg-gray-200 rounded" /> Occupied</span>

...
export function SeatCell({
  ...
  let stateClasses = STATE_STYLES[state] || STATE_STYLES.available;
  if (groupAssignment) {
    if (isActiveGroupMember) {
      stateClasses =
        "bg-ui-primary border-2 border-amber-400 dark:border-amber-300 text-white font-black ring-4 ring-amber-400/60 shadow-md scale-105";
    } else {
      stateClasses =
  ...
  const zIndex = isActiveGroupMember
    ? 25
    : groupAssignment
    ? 10 - colIndex
  ...
  const isDisabledAvailable = disabled && (state === "available" || state === "auto_assigned");
  ...
  return (
    <Component
      ...
      title={`Seat ${seat.seat_label} · ${seat.seat_type} · ${seat.side}${seat.is_accessibility ? " · accessibility-priority" : ""}${groupAssignment ? ` · assigned to ${groupAssignment.member_name}` : ""}${isActiveGroupMember ? " (Currently Selected Passenger)" : ""}${state === "auto_assigned" ? " (AI Recommended)" : ""}${disabled ? " · unavailable for accessibility request" : ""}`}
      aria-label={`Seat ${seat.seat_label}, ${groupAssignment ? `group assignment ${groupAssignment.member_index + 1}, ${groupAssignment.member_name}` : state.replace("_", " ")}${seat.is_accessibility ? ", accessibility priority" : ""}${isActiveGroupMember ? ", currently selected" : ""}${disabled ? ", unavailable for accessibility request" : ""}`}
    >
      {seat.seat_label}
      {groupAssignment ? (
        <span
          className={`absolute -right-1.5 -top-1.5 z-10 pointer-events-none inline-flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-extrabold shadow-md ring-1 ${
            isActiveGroupMember
        ...
        <span
          className="absolute -right-1 -top-1 z-10 pointer-events-none inline-flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white/90 text-[8px] leading-none shadow-sm"
          aria-hidden
        >

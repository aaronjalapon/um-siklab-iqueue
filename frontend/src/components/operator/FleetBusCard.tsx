  ...
  return (
    <article className={`${uiStyles.surface} p-5 flex flex-col gap-3`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className={`${uiStyles.badge} ${surgeColorClass(bus.surge_probability)}`}
        >
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        ...
        {pct >= 90 && !isFull && (
          <span
            className={`${uiStyles.badge} bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200`}
          >
            Nearly full
          </span>
        )}
      ...
      <Link
        ...
        className={`${uiStyles.button} w-full text-sm text-center`}
      >

...
export default function BookPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For the demo, we use a hardcoded passenger_id approach
      // In production, the passenger ID would come from auth
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border p-6">
          ...
          <div className="text-center mb-6">
            <div className="inline-block bg-gray-700 text-white text-xs px-6 py-1 rounded-full">
              🚌 Driver
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4 mt-6 text-xs text-gray-500">
            <span className="flex items-center gap-1">
              <span className="w-4 h-4 bg-blue-100 rounded" /> Available
            </span>
            <span className="flex items-center gap-1">
              <span className="w-4 h-4 bg-gray-200 rounded" /> Occupied
            </span>
            <span className="flex items-center gap-1">
              <span className="w-4 h-4 bg-green-600 rounded" /> Selected
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                value={form.travel_habits}
                onChange={(e) =>
                  setForm({ ...form, travel_habits: e.target.value })
                }
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
              >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, accessibility_needs: e.target.checked })
                }
                className="rounded"
              />
            ...
            {selectedSeat && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm">
                <span className="font-medium">Selected:</span> Seat{" "}
                {selectedSeat}
              </div>
            )}

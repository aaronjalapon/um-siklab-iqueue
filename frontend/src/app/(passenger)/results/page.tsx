    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="text-center py-12 text-gray-500">
          <BusIcon className="w-8 h-8 mx-auto mb-2 animate-pulse" />
      ...
      {!loading && buses.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          <p className="text-lg">No buses found for this route and date.</p>
      ...
      <div className="space-y-4">
        {buses.map((bus) => (
          <div
            key={bus.id}
            className="bg-white rounded-xl shadow-sm border p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
          >
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                {bus.surge_probability !== null && (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${surgeColorClass(bus.surge_probability)}`}
                  >
              ...
              <div className="mt-2">
                <span
                  className={`text-sm font-medium ${bus.available_seats < 5 ? "text-red-600" : "text-green-600"}`}
                >
            ...
            <button
              onClick={() =>
                router.push(`/book/${bus.id}?date=${date}&origin=${encodeURIComponent(origin)}&dest=${encodeURIComponent(destination)}`)
              }
